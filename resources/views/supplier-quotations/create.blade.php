    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('supplier-quotations.store', $canvass) }}" id="quotationForm">
                        @csrf
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <x-text-input id="supplier_contact" class="block mt-1 w-full" type="text" name="supplier_contact" :value="old('supplier_contact')" />
                                        <x-input-error :messages="$errors->get('supplier_contact')" class="mt-2" />
                                    </div>
                                    <div>
                                        <x-input-label for="supplier_email" :value="__('Email')" />
                                        <x-text-input id="supplier_email" class="block mt-1 w-full" type="email" name="supplier_email" :value="old('supplier_email')" />
                                        <x-input-error :messages="$errors->get('supplier_email')" class="mt-2" />
                                    </div>
                                    <div>
                                        <x-input-label for="submitted_date" :value="__('Submitted Date')" />
                                        <x-text-input id="submitted_date" class="block mt-1 w-full" type="date" name="submitted_date" :value="old('submitted_date', date('Y-m-d'))" required />
                                        <x-input-error :messages="$errors->get('submitted_date')" class="mt-2" />
                                    </div>
                                </div>
                                <div class="mt-4">
                                    <x-input-label for="supplier_address" :value="__('Address')" />
                                    <textarea id="supplier_address" name="supplier_address" rows="2" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('supplier_address') }}</textarea>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <x-text-input id="delivery_days" class="block mt-1 w-full" type="number" name="delivery_days" :value="old('delivery_days')" min="0" />
                                        <x-input-error :messages="$errors->get('delivery_days')" class="mt-2" />
                                    </div>
                                    <div class="flex items-center mt-6">
                                        <input id="is_compliant" name="is_compliant" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('is_compliant') ? 'checked' : '' }}>
                                        <x-input-label for="is_compliant" :value="__('Is Compliant')" class="ml-2" />
                                    </div>
                                </div>
                                <div class="mt-4">
                                    <x-input-label for="remarks" :value="__('Remarks')" />
                                    <textarea id="remarks" name="remarks" rows="3" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('remarks') }}</textarea>
                            ...
                            <div>
                                ...
                                <div id="items-container">
                                    @foreach($canvass->rfq->purchaseRequest->prItems as $index => $prItem)
                                        <div class="border rounded-lg p-4 mb-4 item-row" data-item-index="{{ $index }}">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                                <div>
                                                    ...
                                                    <x-text-input :id="'items_' . $index . '_quantity'" class="block mt-1 w-full" type="number" step="0.01" :name="'items[' . $index . '][quantity]'" :value="old('items.' . $index . '.quantity', $prItem->quantity)" required />
                                                ...
                                                <div>
                                                    ...
                                                    <x-text-input :id="'items_' . $index . '_unit_of_measure'" class="block mt-1 w-full" type="text" :name="'items[' . $index . '][unit_of_measure]'" :value="old('items.' . $index . '.unit_of_measure', $prItem->unit_of_measure)" required />
                                                ...
                                                <div>
                                                    ...
                                                    <x-text-input :id="'items_' . $index . '_unit_price'" class="block mt-1 w-full item-unit-price" type="number" step="0.01" :name="'items[' . $index . '][unit_price]'" :value="old('items.' . $index . '.unit_price')" required />
                                            ...
                                            <div class="mt-4">
                                                ...
                                                <x-text-input :id="'items_' . $index . '_item_name'" class="block mt-1 w-full" type="text" :name="'items[' . $index . '][item_name]'" :value="old('items.' . $index . '.item_name', $prItem->item_name)" required />
                                            ...
                                            <div class="mt-4">
                                                ...
                                                <textarea :id="'items_' . $index . '_item_description'" :name="'items[' . $index . '][item_description]'" rows="2" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('items.' . $index . '.item_description', $prItem->item_description) }}</textarea>
