                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('purchase-requests.store') }}" id="prForm">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="project_title" class="block mt-1 w-full" type="text" name="project_title" :value="old('project_title')" :hasError="$errors->has('project_title')" required />
                                    <x-input-error :messages="$errors->get('project_title')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="end_user_department" :value="__('Department')" />
                                    <x-text-input id="end_user_department" class="block mt-1 w-full" type="text" name="end_user_department" :value="old('end_user_department', Auth::user()->department)" :hasError="$errors->has('end_user_department')" required />
                                    <x-input-error :messages="$errors->get('end_user_department')" class="mt-2" />
                                </div>

                                <div>
                                    <x-input-label for="fund_source" :value="__('Fund Source')" />
                                    <x-text-input id="fund_source" class="block mt-1 w-full" type="text" name="fund_source" :value="old('fund_source')" :hasError="$errors->has('fund_source')" required />
                                ...
                                <div>
                                    ...
                                    <div class="mt-1 relative rounded-md shadow-sm">
                                        ...
                                        <x-text-input id="estimated_budget" class="block w-full pl-7" type="number" step="0.01" name="estimated_budget" :value="old('estimated_budget')" :hasError="$errors->has('estimated_budget')" required />
                                    </div>
                                ...
                                <div>
                                    ...
                                    <select id="urgency_level" name="urgency_level" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('urgency_level') border-red-300 @enderror" required>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="approval_date" class="block mt-1 w-full" type="date" name="approval_date" :value="old('approval_date', date('Y-m-d'))" :hasError="$errors->has('approval_date')" required />
                            ...
                            <div class="mt-6">
                                ...
                                <textarea id="project_description" name="project_description" rows="3" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('project_description') border-red-300 @enderror">{{ old('project_description') }}</textarea>
                            ...
                            <div class="mt-6">
                                ...
                                <x-text-input id="urgency_timeline" class="block mt-1 w-full" type="text" name="urgency_timeline" :value="old('urgency_timeline')" placeholder="e.g., Within 30 days" :hasError="$errors->has('urgency_timeline')" />
                        ...
                        <div class="mb-8">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <label class="flex items-center">
                                    <input type="checkbox" name="has_signatures" value="1" {{ old('has_signatures') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                    <span class="ml-2 text-sm text-gray-600">Signatures</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="has_specs" value="1" {{ old('has_specs') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                    <span class="ml-2 text-sm text-gray-600">Specifications</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="has_quantity" value="1" {{ old('has_quantity') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                    <span class="ml-2 text-sm text-gray-600">Quantities</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="has_market_survey" value="1" {{ old('has_market_survey') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                        ...
                        <div class="mb-8">
                            ...
                            <textarea id="deficiency_notes" name="deficiency_notes" rows="3" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('deficiency_notes') border-red-300 @enderror">{{ old('deficiency_notes') }}</textarea>
                            <x-input-error :messages="$errors->get('deficiency_notes')" class="mt-2" />
                        </div>

                        <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-200">
                            <a href="{{ route('purchase-requests.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition">
                                Cancel
                            </a>
                            <x-primary-button>
