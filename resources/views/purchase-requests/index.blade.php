    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Manage and track all purchase requests</p>
            </div>
        ...
        <x-card>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge 
                                            ...
                                            @elseif(in_array($pr->status, ['RFQ_READY', 'RFQ_DISSEMINATED'])) badge-info
                                            @elseif(in_array($pr->status, ['CANVASS_COMPLETE', 'BAC_DOCS_READY'])) badge-info
                                            @elseif(in_array($pr->status, ['BAC_APPROVED', 'PO_APPROVED'])) badge-success
                                    ...
                                    <td>
                                        <div class="flex items-center space-x-3">
                                            <a href="{{ route('purchase-requests.show', $pr) }}" class="text-indigo-600 hover:text-indigo-700 text-sm font-medium">View</a>
                                            @if(Auth::user()->hasAnyRole(['END_USER', 'PROCUREMENT_OFFICER', 'ADMIN']) && 
                                                ($pr->end_user_id == Auth::id() || Auth::user()->hasAnyRole(['PROCUREMENT_OFFICER', 'ADMIN'])))
                                                @if($pr->status == 'PR_UNDER_REVIEW')
                                                    <a href="{{ route('purchase-requests.edit', $pr) }}" class="text-yellow-600 hover:text-yellow-700 text-sm font-medium">Edit</a>
                                                @endif
                                            @endif
                                        </div>
                                    </td>
            ...
            @else
                <div class="empty-state">
                    ...
                    <h3 class="empty-state-title">No purchase requests</h3>
                    <p class="empty-state-description">Get started by creating a new purchase request.</p>
                    @if(Auth::user()->hasAnyRole(['END_USER', 'PROCUREMENT_OFFICER', 'ADMIN']))
                        <div class="mt-6">
                            <a href="{{ route('purchase-requests.create') }}" class="btn btn-primary">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
