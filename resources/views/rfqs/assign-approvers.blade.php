<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Assign Approvers: RFQ ') . $rfq->rfq_number }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <div class="mb-4">
                        <a href="{{ route('rfqs.show', $rfq) }}" class="text-indigo-600 hover:text-indigo-900">
                            ← Back to RFQ
                        </a>
                    </div>

                    <div class="mb-6 bg-blue-50 border border-blue-200 rounded-md p-4">
                        <h3 class="text-lg font-medium text-gray-900 mb-2">RFQ Information</h3>
                        <p class="text-sm text-gray-600">RFQ Number: <strong>{{ $rfq->rfq_number }}</strong></p>
                        <p class="text-sm text-gray-600">PR Number: <strong>{{ $rfq->purchaseRequest->pr_number }}</strong></p>
                        <p class="text-sm text-gray-600">Project Title: <strong>{{ $rfq->purchaseRequest->project_title }}</strong></p>
                        <p class="text-sm text-gray-600">Status: <strong>{{ $rfq->status_name }}</strong></p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('rfqs.store-approvers', $rfq) }}" id="approversForm">
                        @csrf
                        ...
                        <div class="mt-6 flex items-center justify-end space-x-4">
                            <a href="{{ route('rfqs.show', $rfq) }}" class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                                Cancel
                            </a>
    ...
    <script>
        let approverCount = 0;
        const approvers = @json($approvers->map(function($u) { return ['id' => $u->id, 'name' => $u->name, 'role' => $u->role_name]; }));
        ...
        function addApproverField() {
            ...
            approvers.forEach(function(approver) {
                optionsHtml += '<option value="' + approver.id + '">' + approver.name + ' (' + approver.role + ')</option>';
            });
